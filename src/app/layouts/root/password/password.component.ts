...
export class PasswordComponent {
  ...
  public async change() {
    const res = await this._httpService.put('users/current/password', {
      ...
      if (err.status == 422) {
        this._toastService.show(this._i18nService.translate(`layouts.root.password.invalid.${err.error}`));
      }
    }) !== undefined;
    if (res) {
      this._dialogRef.close();
      await this._httpService.delete('sessions', { reason: 'Modify Password' });
      await this._dialogService.alert(this._i18nService.translate('layouts.root.password.success'));
