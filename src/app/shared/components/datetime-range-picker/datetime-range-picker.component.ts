...
import { MAT_RANGE_DATE_SELECTION_MODEL_PROVIDER, MatDateRangePicker } from '@angular/material/datepicker';
import { take } from 'rxjs';
import { DatetimepickerContent } from '../datetimepicker-content/datetimepicker-content.component';

@Component({
  selector: 'mat-datetime-range-picker',
  template: '',
  exportAs: 'matDatetimeRangePicker',
  changeDetection: ChangeDetectionStrategy.OnPush,
  encapsulation: ViewEncapsulation.None,
  providers: [
    MAT_RANGE_DATE_SELECTION_MODEL_PROVIDER,
    {provide: MatDateRangePicker, useExisting: DatetimeRangePicker}
  ]
})
export class DatetimeRangePicker<D> extends MatDateRangePicker<D> implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    ...
    const datepickerInputStart = datepickerInput._startInput;
    const datepickerInputEnd = datepickerInput._endInput;
    const format = JSON.parse(JSON.stringify(datepickerInputStart._dateFormats));
    format.display.dateInput = format.display.datetimeInput;
    datepickerInputStart._dateFormats = format;
    datepickerInputEnd._dateFormats = format;
  }
}